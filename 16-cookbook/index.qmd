...
gamelogs |>  pull(pts) |>  
  ...
  hc_title(text = "23-24 Box Score Points")
```
...
gamelogs |>  
  ...
  hc_title(text = "23-24 Box Score Fouls")
```
...
gamelogs |> 
  ...
  hc_subtitle(text = "Scoring 23-24 Season", style = list(color = lakersGold)) |> 
  hc_chart(
...
gamelogs |> 
  ...
  hc_chart(
    backgroundColor = list(
       ...
       stops = list(
         list(0, 'rgba(0, 0, 0, 0.8'),             # Start Color (black)
...
baseBronChart <- 
  ...
  hc_yAxis(
    ...
    plotLines = list(
      list(                # Defines a single plot line, could add more
        ...
        label = list(
          text = glue("Season ppg: {round(bronPPG,1)}"), # Text on the plotline
          style = list(
  ...
  hc_xAxis(
    ...
    plotBands = list(
        list(
          from = 55,                        # Start of the plotband (first game # of injury)
          to = 58,                          # End of the plotband (last game missed)
          color = "rgba(85, 37, 131, 0.2)", # RGB specification of the lakers purple color with a 20% alpha (transparency)
          label = list(
            text = "???",          # Text for the plotBand
            style = list(fontWeight = "bold", color = "#FFFFFF", fontSize = "12px")
          )
        )
    )
  )
...
bronChartData <- 
  ...
  mutate(
    hc_dataLabel = case_when(
      pts == max(pts) ~ glue("High Score {pts} <i> ({dateGame} vs {slugOpponent}) </i>"),
      pts == min(pts) ~ glue("Low Score {pts} <i> ({dateGame} vs {slugOpponent}) </i>"),
      TRUE ~ "" # No label
    )
  )
...
gamelogs |> 
  ...
  hc_xAxis(
    plotLines = list(
      list(
        value = ymd(20231225) %>% datetime_to_timestamp(),
        color = "#FF0000",
        label = list(
          text = "xmas day games",
          align = "top",
