...
images <- data.frame(
  ...
  mutate(text_label = glue("<span style='font-size:14px;'>**{toupper(label)}**</span><br>
                           ...
                           <span style='font-size:12.5px;color:grey40;'>{total} shot attempts</span><br>
...
draft_chart_first_game <- base_chart_first_game + 
  ...
  ggtext::geom_textbox(data = first_game_images, aes(x = -15, y = -7, label = text_label), 
                       fill = NA, box.size = NA) +
...
draft_chart_yr_twenty_game <- base_chart_yr_twenty_game + 
  ...
  ggtext::geom_textbox(data = year_twenty_game_images, aes(x = -15, y = -7, label = text_label), 
                       fill = NA, box.size = NA) +
